{
  ...
  function buildNullCoalescingExpression(first, rest) {
    return buildTree(first, rest, function(result, element) {
      return new ast.NullCoalescingExpression(result, element[3]);
    });
  }
...
IdentifierName "identifier"
  = first:IdentifierStart rest:IdentifierPart* {
      return new ast.Identifier(first + rest.join(""));
    }
...
HexIntegerLiteral
  = "0x"i digits:$HexDigit+ { return new NumberLiteral(text(), 16); }
...
StringLiteral "string"
  = '"' chars:DoubleStringCharacter* '"' {
      return new StringLiteral(chars.join(""));
    }
...
Program
  = body:StatementList? {
      return new ast.Program(optionalList(body));
    }
...
Block
  = "{" __ body:(StatementList __)? "}" {
      return new ast.BlockStatement(optionalList(extractOptional(body, 0)));
    }
    
VariableStatement
  = VarToken __ declarations:VariableDeclarationList EOS {
      return new ast.VariableDeclarationStatement(declarations);
    }
...
VariableDeclaration
  = id:Identifier init:(__ Initialiser)? {
      return new ast.VariableDeclarator(id, extractOptional(init, 1));
    }
...
FunctionDeclaration
  = FuncToken __ id:Identifier __
    ...
    __ body:Block __ 
    {
      return new ast.FunctionDeclarationStatement(id, optionalList(extractOptional(params, 0)), body);
    }
...
IfStatement
  ...
  / IfToken __ test:Expression __
    consequent:Statement {
      return new ast.IfStatement(test, consequent, null);
    }
...
UseStatement
  = UseToken __ id:Identifier EOS
    { return new ast.UseStatement(id) }
...
AssignmentExpression
  = left:LeftHandSideExpression __
    ...
    { return new ast.AssignmentExpression(left, right); }
  / LogicalORExpression
...
RelationalExpression
  = first:NullCoalescingExpression
    rest:(__ RelationalOperator __ NullCoalescingExpression)*
    { return buildBinaryExpression(first, rest); }
...
UnaryExpression
  ...
  / operator:UnaryOperator __ argument:UnaryExpression {
      if (operator === "++" || operator === "--") {
        return new ast.UpdateExpression(argument, operator, true);
      } else {
...
PostfixExpression
  = argument:ExistentialExpression _ operator:PostfixOperator {
      return new ast.UpdateExpression(argument, operator, false);
    }
...
ExistentialExpression
  = argument:LeftHandSideExpression "?" !"?" {
    return new ast.ExistentialExpression(argument);
  }
  ...
  = first:(
      callee:MemberExpression __ nullPropagatingOperator:"?"? __ args:Arguments {
        if (nullPropagatingOperator === "?") {
          return new ast.NullCheckCallExpression(callee, args);
        } else {
    ...
    )*
    {
      return buildTree(first, rest, function(result, element) {
        if (element.type === "MemberExpression") {
          return new ast.MemberExpression(result, element.property, element.computed);
        } if (element.type === "NullPropagatingExpression") {
          return new ast.NullPropagatingExpression(result, element.property, element.computed);
        } else if (element.type === "CallExpression") {
          return new ast.CallExpression(result, element.arguments);
        } else if (element.type === "NullCheckCallExpression") {
          return new ast.NullCheckCallExpression(result, element.arguments);
        }
      });
    }
    ...
    rest:(
      ...
      / __ nullPropagatingOperator:"?"? __ "." __ property:IdentifierName {
          return { 
            property: property, 
            computed: false, 
            isNullPropagating: nullPropagatingOperator === "?"
          };
        }
    )*
    {
      return buildTree(first, rest, function (result, element) {
        if (element.isNullPropagating) {
          return new ast.NullPropagatingExpression(result, element.property, element.computed);
        } else {
          return new ast.MemberExpression(result, element.property, element.computed);
        }
...
FunctionExpression
  ...
  / "(" __ params:(FormalParameterList __)? ")" __ FunctionExpressionOperator
    __ body:Block __
    {      
      return new ast.FunctionExpression(
        null,
...
BooleanLiteral
  = TrueToken  { return new ast.BooleanLiteral("true"); }
  / FalseToken { return new ast.BooleanLiteral("false"); }
...
DecimalLiteral
  = DecimalIntegerLiteral "." DecimalDigit* ExponentPart? {
      return new ast.NumberLiteral(text());
    }
  / "." DecimalDigit+ ExponentPart? {
      return new ast.NumberLiteral(text());
    }
...
StringLiteral "string"
  = '"' chars:DoubleStringCharacter* '"' {
      return new ast.StringLiteral(chars.join(""));
    }
  / "'" chars:SingleStringCharacter* "'" {
      return new ast.StringLiteral(chars.join(""));
    }
...
ArrayLiteral
  = "[" __ elision:(Elision __)? "]" {
      return new ast.ArrayExpression(optionalList(extractOptional(elision, 0)));
    }
  / "[" __ elements:ElementList __ "]" {
      return new ast.ArrayExpression(elements);
    }
  / "[" __ elements:ElementList __ "," __ elision:(Elision __)? "]" {
      return new ast.ArrayExpression(elements.concat(optionalList(extractOptional(elision, 0))));
    }
...
PropertyAssignment
  = key:PropertyName __ ":" __ value:AssignmentExpression {
      return new ast.Property(key, value);
    }
