...
FunctionDeclaration
  = FuncToken __ id:Identifier __
    ...
    __ body:Block __ 
    {
      return insertLocationData(new ast.FunctionDeclarationStatement(id, optionalList(extractOptional(params, 0)), body), text(), line(), column());
    }
...
AssignmentExpression
  = left:LeftHandSideExpression __
    ...
    { return insertLocationData(new ast.AssignmentExpression(left, right), text(), line(), column()); }
  / LogicalORExpression
...
StringLiteral "string"
  = '"' chars:DoubleStringCharacter* '"' {
      return insertLocationData(new ast.StringLiteral(chars.join("")), text(), line(), column());
    }
  / "'" chars:SingleStringCharacter* "'" {
      return insertLocationData(new ast.StringLiteral(chars.join("")), text(), line(), column());
    }
